...
Fx = require './td'
local AFROTOOtt =  require('tdlua') 
local client = AFROTOOtt()
local tdf = Fx.xnxx()
...
local AFROTOO = io.open("AFROTOO", 'w')
AFROTOO:write([[
cd $(cd $(dirname $0); pwd)
lua5.2 AFROTOO.lua
]])
...
os.execute('chmod +x AFROTOO;chmod +x AFROTOO;./AFROTOO')
end
...
AFROTOO = Token:match("(%d+)")
os.execute('sudo rm -fr .CallBack-Bot/'..AFROTOO)
bot = Fx.AFROTOObots.set_config{api_id=1846213,api_hash='c545c613b78f18a30744970910124d53',session_name=AFROTOO,token=Token}
function var(value)
...
function Dev(msg) 
if Redis:sismember(AFROTOO.."Dev",msg.sender_id.user_id) or tonumber(msg.sender_id.user_id) == tonumber(Sudo_Id) then
ok = true
else
ok = false
end
...
if tonumber(data.sender_id.user_id) == tonumber(AFROTOO) then
return false
end
end
function ChannelJoin(id)
JoinChannel = true
local chh = Redis:get(AFROTOO.."chfalse")
if chh then
...
if data.sender_id.user_id == tonumber(AFROTOO) then
return false
end
if Redis:get(AFROTOO.."chsource") then
chsource = Redis:get(AFROTOO.."chsource")
else
chsource = "Pp_g3"
end
...
local Info_User = Redis:get(AFROTOO.."Twasl:UserId"..Message_Get.forward_info.date) or 46899864
if text == 'حظر' then
Redis:sadd(AFROTOO..'Ban:users',Info_User)  
return send(data.chat_id,data.id,Reply_Status(Info_User,'≋ تم حظره من الصانع').Reply,"md",true)  
end 
if text =='الغاء الحظر' or text =='الغاء حظر' then
Redis:srem(AFROTOO..'Ban:users',Info_User)  
return send(data.chat_id,data.id,Reply_Status(Info_User,'≋ تم الغاء حظره من الصانع ').Reply,"md",true)  
end 
end
end
if text == "≋ الغاء الامر" then
Redis:del(AFROTOO..data.sender_id.user_id.."bottoken")
Redis:del(AFROTOO..data.sender_id.user_id.."botuser")
Redis:del(AFROTOO..data.sender_id.user_id.."make:bot")
...
Redis:sadd(AFROTOO.."Dev",text:match("^رفع مطور (%d+)$"))
send(data.chat_id,data.id,'≋ تم رفع العضو مطور ف الصانع بنجاح ',"md",true)  
return false 
end
if text and text:match("^تنزيل مطور (%d+)$") then
Redis:srem(AFROTOO.."Dev",text:match("^تنزيل مطور (%d+)$"))
send(data.chat_id,data.id,'≋ تم تنزيل العضو مطور من الصانع بنجاح ',"md",true)  
return false 
end

if text == "≋ تفعيل الوضع المجاني" then 
Redis:del(AFROTOO.."free:bot")
send(data.chat_id,data.id,'≋ تم تفعيل الوضع المجاني ',"md",true)  
end
if text == "≋ تعطيل الوضع المجاني" then 
Redis:set(AFROTOO.."free:bot",true)
send(data.chat_id,data.id,'≋ تم تعطيل الوضع المجاني ',"md",true)  
end
-----تشغيل البوتات ---
if text and Redis:get(AFROTOO..data.sender_id.user_id.."run:bot") then
Redis:del(AFROTOO..data.sender_id.user_id.."run:bot")
Redis:del(AFROTOO.."screen:on")
Redis:del(AFROTOO.."bots:folder")
userbot = text:gsub("@","")
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
Redis:sadd(AFROTOO.."bots:folder",folder:gsub("@",""))
end
end
if not Redis:sismember(AFROTOO.."bots:folder",userbot) then
...
Redis:sadd(AFROTOO.."screen:on",screen)
end
local list = Redis:smembers(AFROTOO..'screen:on')
...
Redis:set(AFROTOO..data.sender_id.user_id.."run:bot",true)
send(data.chat_id,data.id,"≋ ارسل معرف البوت ليتم تشغيله")
return false 
end
---ايقاف البوتات
if text and Redis:get(AFROTOO..data.sender_id.user_id.."stop:bot") then
Redis:del(AFROTOO..data.sender_id.user_id.."stop:bot")
Redis:del(AFROTOO.."screen:on")
Redis:del(AFROTOO.."bots:folder")
userbot = text:gsub("@","")
for folder in io.popen('ls'):lines() do
...
Redis:set(AFROTOO..data.sender_id.user_id.."stop:bot",true)
send(data.chat_id,data.id,"≋ ارسل معرف البوت ليتم ايقافه")
return false 
end
--الاشتراك الاجباري 
if Redis:get(AFROTOO.."ch:addd"..data.sender_id.user_id) == "on" then
Redis:set(AFROTOO.."ch:addd"..data.sender_id.user_id,"off")
...
Redis:del(AFROTOO.."chfalse")
Redis:set(AFROTOO.."chfalse",ch)
Redis:del(AFROTOO.."ch:admin")
Redis:set(AFROTOO.."ch:admin",da.result.invite_link)
else
send(data.chat_id,data.id,'≋ المعرف خطأ او البوت ليس مشرف في القناه ',"md",true)  
end
end
if text == "≋ تفعيل الاشتراك الاجباري" then
Redis:set(AFROTOO.."ch:addd"..data.sender_id.user_id,"on")
...
if text and Redis:get(AFROTOO..data.sender_id.user_id.."make:bot") == "devuser" then
...
local bottoken = Redis:get(AFROTOO..data.sender_id.user_id.."bottoken")
local botuser = Redis:get(AFROTOO..data.sender_id.user_id.."botuser")
...
Redis:sadd(AFROTOO.."userbots",botuser)
Redis:del(AFROTOO..data.sender_id.user_id.."bottoken")
Redis:del(AFROTOO..data.sender_id.user_id.."botuser")
Redis:del(AFROTOO..data.sender_id.user_id.."make:bot")
...
if text and Redis:get(AFROTOO..data.sender_id.user_id.."make:bot") == "token" then
...
if Redis:sismember(AFROTOO.."userbots",botuser) then
send(data.chat_id,data.id, "\n≋ عذرا هذا البوت مصنوع بالفعل","md",true)  
return false 
end 
Redis:set(AFROTOO..data.sender_id.user_id.."botuser",botuser)
Redis:set(AFROTOO..data.sender_id.user_id.."bottoken",text)
Redis:set(AFROTOO..data.sender_id.user_id.."make:bot","devuser")
...
Redis:set(AFROTOO..data.sender_id.user_id.."make:bot","token")
...
if text == "≋ فحص" then
Redis:del(AFROTOO.."All:pv:st")
Redis:del(AFROTOO.."All:gp:st")
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
m = Redis:get(folder)
x = {m:match("(.*)&(.*)$(.*)+(.*)")}
...
list = Redis:smembers(bot_id.."ChekBotAdd") 
lt = Redis:smembers(bot_id.."Num:User:Pv") 
Redis:incrby(AFROTOO.."All:gp:st",tonumber(#list))
Redis:incrby(AFROTOO.."All:pv:st",tonumber(#lt))
end
end
send(data.chat_id,data.id,'\n≋ احصائيات جميع البوتات المصنوعه \n ≋ عدد المجموعات '..Redis:get(AFROTOO.."All:gp:st")..' مجموعه \n≋ عدد المشتركين '..Redis:get(AFROTOO.."All:pv:st")..' مشترك',"html",true)
end
-----ban all
...
Redis:del(AFROTOO..'3ddbots')
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
os.execute('cd '..folder..' && chmod +x * && screen -d -m -S '..folder:gsub("@","")..' ./Run')
Redis:sadd(AFROTOO..'3ddbots',folder)
end
end
local list = Redis:smembers(AFROTOO..'3ddbots')
...
Redis:del(AFROTOO.."fake")
Redis:del(AFROTOO..'Ban:users')  
...
-------delete 
if text and Redis:get(AFROTOO..data.sender_id.user_id.."make:bot") == "del" then
...
Redis:srem(AFROTOO.."userbots",text:gsub("@",""))
send(data.chat_id,data.id, "\n≋ تم حذف البوت بنجاح","md",true)  
return false 
end 
if text == "≋ حذف بوت" then
Redis:set(AFROTOO..data.sender_id.user_id.."make:bot","del")
...
devbot = x[3]
bottoken = x[4]
Redis:sadd(AFROTOO..'3ddbots',botuser..' » '..devbot)
end
end
local list = Redis:smembers(AFROTOO..'3ddbots')
if #list <= 100 then
...
----end--3dd
if text and Redis:get(AFROTOO..data.sender_id.user_id.."setchannel") then
if text == "الغاء" or text == '≋ الغاء الامر' then   
Redis:del(AFROTOO..data.sender_id.user_id.."setchannel")
send(data.chat_id,data.id, "\n≋ تم الغاء تعيين قناه السورس","md",true)  
return false 
end 
if text:match("@(.*)") then
local ch = text:match("@(.*)")
Redis:set(AFROTOO.."chsource",ch)
...
if tonumber(AFROTOO) ~= tonumber(FilesJson.BotId) then
return send(msg_chat_id,msg_id,'≋ عذرا هاذا الملف غير مطابق مع البوت يرجى جلب النسخه الحقيقيه')
end -- end botid
...
Y = Y + 1
Redis:sadd(AFROTOO..'total',v)  
end
end
end
if text == "جلب نسخه" then
local UsersBot = Redis:smembers(AFROTOO.."total")
local Get_Json = '{"BotId": '..AFROTOO..','  
if #UsersBot ~= 0 then 
...
----brodcast all
if Redis:get(AFROTOO.."all:texting") then
if text == "الغاء" or text == '≋ الغاء الامر' then   
Redis:del(AFROTOO.."all:texting")
send(data.chat_id,data.id, "\n≋ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(AFROTOO.."3z:gp",text)
Redis:del(AFROTOO.."all:texting")
...
list = Redis:smembers(bot_id.."ChekBotAdd") 
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(AFROTOO.."3z:gp")).."&parse_mode=Markdown")
end
end
end
Redis:del(AFROTOO.."3z:gp")
Redis:del(AFROTOO.."all:texting")
send(data.chat_id,data.id,"≋ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if Redis:get(AFROTOO.."all:texting:pv") then
if text == "الغاء" or text == '≋ الغاء الامر' then   
Redis:del(AFROTOO.."all:texting:pv")
send(data.chat_id,data.id, "\n≋ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(AFROTOO.."eza3a:pv",text)
Redis:del(AFROTOO.."all:texting:pv")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(AFROTOO.."eza3a:pv")).."&parse_mode=Markdown")
end
end
end
Redis:del(AFROTOO.."eza3a:pv")
Redis:del(AFROTOO.."all:texting:pv")
send(data.chat_id,data.id,"≋ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if text == "≋ اذاعه عام للمجموعات" then
Redis:set(AFROTOO.."all:texting",true)
send(data.chat_id,data.id,"ارسل النص الان","html",true)
end
if text == "≋ اذاعه عام للمشتركين" then
Redis:set(AFROTOO.."all:texting:pv",true)
send(data.chat_id,data.id,"ارسل النص الان","html",true)
end
--brodcast
if Redis:get(AFROTOO..data.sender_id.user_id.."brodcast") then 
if text == "الغاء" or text == '≋ الغاء الامر' then   
Redis:del(AFROTOO..data.sender_id.user_id.."brodcast") 
send(data.chat_id,data.id, "\n≋ تم الغاء الاذاعه","md",true)  
return false 
end 
local list = Redis:smembers(AFROTOO.."total") 
...
if text == "≋ اذاعه" then
Redis:set(AFROTOO..data.sender_id.user_id.."brodcast",true)
send(data.chat_id,data.id,"≋ ارسل الاذاعه الان")
return false 
end
---fwd
if Redis:get(AFROTOO..data.sender_id.user_id.."brodcast:fwd") then 
if text == "الغاء" or text == '≋ الغاء الامر' then   
Redis:del(AFROTOO..data.sender_id.user_id.."brodcast:fwd")
...
Redis:set(AFROTOO..data.sender_id.user_id.."brodcast:fwd",true)
...
chinfo = Redis:get(AFROTOO.."ch:admin")
var(bot.sendText(msg.chat_id,msg.id,'\n≋ عليك الاشتراك في قناة البوت لاستخذام الاوامر\n\n'..chinfo..''))
return false 
end

if not Redis:get(AFROTOO.."twsl") then
if msg.sender_id.user_id ~= tonumber(AFROTOO) then
if Redis:sismember(AFROTOO..'Ban:users',msg.sender_id.user_id) then
return false 
end
if msg.id then
Redis:setex(AFROTOO.."Twasl:UserId"..msg.date,172800,msg.sender_id.user_id)
...
if text and Redis:get(AFROTOO.."free:bot") then
return send(msg.chat_id,msg.id,"≋ الوضع المجاني معطل من قبل مطور الصانع")
end
if text == "/start" then
if not Redis:sismember(AFROTOO.."total",msg.sender_id.user_id) then
Redis:sadd(AFROTOO.."total",msg.sender_id.user_id)
end
reply_markup = bot.replyMarkup{type = 'keyboard',resize = true,is_personal = true,
data = {
{
...
send(msg.chat_id,msg.id,"≋ اهلا بك في صانع بوتات سورس ارتكس \n≋ مطور البوت @IIUll_l","html",true, false, false, true, reply_markup)
return false 
end
---making user
if text and Redis:get(AFROTOO..msg.sender_id.user_id.."make:bot") then
if text == "≋ الغاء" then
Redis:del(AFROTOO..msg.sender_id.user_id.."make:bot")
...
Informationlua:close()
os.execute('cp -a ./source/. ./@'..botuser..' && cd @'..botuser..' && chmod +x * && screen -d -m -S '..botuser..' ./Run')
Redis:set(AFROTOO..msg.sender_id.user_id.."my:bot",botuser)
Redis:sadd(AFROTOO.."userbots",botuser)
Redis:del(AFROTOO..msg.sender_id.user_id.."make:bot")
...
if text == "≋ صنع بوت" then
if Redis:get(AFROTOO..msg.sender_id.user_id.."my:bot") then
return send(msg.chat_id,msg.id,"≋ عفوا لديك بوت بالفعل احذفه اولا")
end
Redis:set(AFROTOO..msg.sender_id.user_id.."make:bot",true)
...
if Redis:get(AFROTOO..msg.sender_id.user_id.."my:bot") then
local botuser = Redis:get(AFROTOO..msg.sender_id.user_id.."my:bot")
...
Redis:srem(AFROTOO.."userbots",botuser)
Redis:del(AFROTOO..msg.sender_id.user_id.."my:bot")
send(msg.chat_id,msg.id, "\n≋ تم حذف البوت بنجاح","md",true)  
else
...
if data and data.AFROTOObots and data.AFROTOObots == "updateNewMessage" then
if tonumber(data.message.sender_id.user_id) == tonumber(AFROTOO) then
return false
end
Run(data.message,data.message)
elseif data and data.AFROTOObots and data.AFROTOObots == "updateMessageEdited" then
local Message_Edit = bot.getMessage(data.chat_id, data.message_id)
if Message_Edit.sender_id.user_id == AFROTOO then
return false
end
Run(Message_Edit,Message_Edit)
elseif data and data.AFROTOObots and data.AFROTOObots == "updateNewCallbackQuery" then
